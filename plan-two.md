...
## 2026-04-16 — Production Plan Sprint 2 landed (Economy Lock + Minimum Telemetry)

The central metaphor is now rigorously honest: **1 token = 1 second of training**, visible to the player at 1 Hz on the HUD counter. Ships same session as Sprint 1.

**Delivered:**
- **TIME_PACKAGES rewrite to 1:1.** Quick 15/15, Standard 30/30, Extended 60/60, Marathon 180/180. `TUTORIAL_TIME_PACKAGE` and deprecated `COMPUTE_TIERS` / `TRAINING_BASE_*` constants deleted.
- **Per-soldier weapon manuals** (`WEAPON_MANUAL_COST` in `config/roster.ts`): grenade 80, rocket 150, MG 150, tank 300. One-time fee per soldier per rare weapon; 1:1 training per-second preserved after. `WEAPON_TRAINING.tokenCost` additive retired entirely. `campTrainingStore.commitToTrain` charges the manual fee first (with distinct `weapon-manual` sync reason), then the training cost.
- **`LEARN WEAPON` UI preamble** in `TrainingSheet` and `SoldierSheet`: shows "FIRST TIME" badge + soldier name + "must learn {weapon}" + fee when the manual is unpaid. START button copy becomes "`LEARN + START — {total} TOKENS`" or "`START — N TOKENS / N SEC`" to make the 1:1 rate legible on the button itself. New CSS: `.training-manual-notice`, `.training-manual-badge`, `.training-cost-breakdown`.
- **Daily simplified** to flat 150 tokens every 24h. Retired: `DAILY_DRIP_*`, `DAILY_STREAK_REWARDS`, `DAILY_STREAK_FORGIVENESS`, `dailyStreak`, `lastDailyClaimDate`, `lastDailyClaimTime`, `claimDailyTokens()`, `claimDailyReward()` (streak variant). Replaced with `DAILY_GRANT`, `DAILY_COOLDOWN_MS`, `lastDailyClaimMs`, `claimDaily()`, `canClaimDaily()`, `msUntilNextDaily()`. `DailyRewardPopup` rewritten to a single-card layout ("A full day of training on the house", one hero tile, one button). `TokenCounter` wallet tooltip shows "RATE: 1 TOKEN = 1 SECOND" instead of streak info.
- **Starter balance 500 → 200** for brand-new profiles. Existing balances preserved by the v14 migration.
- **Visual honesty (Sprint 2.5) — the headline feature.** During any active training run, the HUD token counter ticks down at 1 Hz, in sync with the `ProgressRing` above the Training Camp building. Implementation: the counter's displayed value = `actual tokens + sum(slot.tokenTotal - slot.tokenBurned for running slots)`. Actual balance drops at commit (the honest deduction); the displayed value shows the in-flight tokens "burning" at exactly 1/sec. Label swaps from TOKENS → TRAINING during a run. Cyan pulse animation (`.token-hero--burning`) applied. Verified in preview: at t=8.08s counter showed 6912, at t=11.06s showed 6909 — **3 tokens ticked in 3 seconds, exactly 1 Hz.**
- **campStore v13 → v14 migration.** Drops `dailyStreak` / `lastDailyClaimDate` / `lastDailyClaimTime`, converges onto `lastDailyClaimMs`. Grandfathers soldiers: any weapon in `trainedBrains` counts as manual-paid (no retroactive charges). Existing token balances preserved. Added `grantWeaponManual(soldierId, weapon)` action; `SoldierRecord.weaponManualsPurchased: WeaponType[]`.
- **Minimum-viable telemetry floor** (Sprint 2.6). New `src/analytics/` with `events.ts` (10 typed events) and `telemetry.ts` (PostHog + Sentry init, env-gated). Ten events wired end-to-end: `boot`, `tutorial_complete`, `training_start`, `training_complete`, `weapon_manual_purchase`, `battle_start`, `battle_complete`, `daily_claimed`, `store_opened`, `pack_viewed`, `pack_clicked`. Env vars: `VITE_POSTHOG_KEY`, `VITE_POSTHOG_HOST`, `VITE_SENTRY_DSN`. All no-op when unset (telemetry failures never break the app). `posthog-js` + `@sentry/browser` installed as deps.
- **`docs/economy.md` F2P progression spreadsheet.** Day-by-day model through Day 60 showing earnings vs endgame cost (~7,080 tokens for fully-kitted 6-soldier squad). First soft wall around Day 10-14 (one $2.99 Charge pack closes it). Endgame reachable F2P in ~45-60 days, or one $9.99 Arsenal pack + a couple weeks. Includes "revisit triggers" — the 4 telemetry signals that should re-open economy tuning.

**Pack values unchanged.** Pack tuning is explicitly deferred to Sprint 3 once 2+ weeks of real telemetry exist — no guessing when data is coming.

**Verified end-to-end:** typecheck clean, 44/44 tests green, production build clean. Preview test committed a 30-token / 30-second standard rifle training run and observed the HUD counter tick down 1 per second in real time. Daily popup redesigned and claiming yields exactly +150 with correct cooldown. Telemetry events fire to dev console when keys unset; ready for PostHog keys to flip on at any time.
